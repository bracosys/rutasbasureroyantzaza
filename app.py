...
def create_app():
    ...
    # Reemplaza tu función driver_navigate existente con esta versión corregida:
    ...
    def driver_navigate(route_id):
        try:
            ...
            # Buscar cualquier ruta en progreso del chofer
            in_progress = RouteCompletion.query.filter_by(
                driver_id=current_user.id,
                status='in_progress'
            ).first()
            ...
            # Verificar que hay vehículo asignado
            if not in_progress.vehicle:
                flash('Error: No hay vehículo asignado para esta ruta.', 'danger')
                return redirect(url_for('driver_dashboard'))
            
            print(f"Navegando ruta: {route.name} (ID: {route.id})")
            print(f"Completion ID: {in_progress.id}")
            print(f"Vehículo: {in_progress.vehicle.brand} {in_progress.vehicle.model}")
            print(f"Combustible inicial: {in_progress.fuel_start}")
    ...
    @app.route('/api/route/navigation-data/<int:route_id>')
    @login_required
    def get_route_navigation_data(route_id):
        """API para obtener datos de navegación de una ruta"""
        try:
            route = Route.query.get_or_404(route_id)
            
            # Verificar permisos
            if not (current_user.is_admin or current_user.is_coordinator or current_user.is_driver):
                return jsonify({'error': 'Sin permisos'}), 403
            
            # Intentar cargar coordenadas desde diferentes fuentes
            coordinates = []
            
            # 1. Si tiene archivo GPX, cargar desde ahí
            if route.gpx_path and os.path.exists(route.gpx_path):
                try:
                    from services.route_optimizer import AdvancedRouteOptimizer
                    optimizer = AdvancedRouteOptimizer()
                    coordinates = optimizer.load_gpx_points(route.gpx_path)
                    print(f"Loaded {len(coordinates)} points from GPX file")
                except Exception as e:
                    print(f"Error loading GPX: {e}")
            
            # 2. Si no hay GPX, intentar extraer del archivo HTML
            if not coordinates and route.file_path and os.path.exists(route.file_path):
                try:
                    coordinates = extract_coordinates_from_html(route.file_path)
                    print(f"Extracted {len(coordinates)} points from HTML file")
                except Exception as e:
                    print(f"Error extracting from HTML: {e}")
            
            # 3. Si tenemos puntos de inicio y fin, crear una ruta básica
            if not coordinates and route.start_point and route.end_point:
                try:
                    start_coords = [float(x) for x in route.start_point.split(',')]
                    end_coords = [float(x) for x in route.end_point.split(',')]
                    coordinates = [start_coords, end_coords]
                    print(f"Created basic route with start/end points")
                except Exception as e:
                    print(f"Error creating basic route: {e}")
            ...
            if coordinates:
                ...
                return jsonify({
                    'success': False,
                    'error': 'No se pudieron cargar las coordenadas de la ruta'
                }), 404
                
        except Exception as e:
            print(f"Error en get_route_navigation_data: {e}")
            return jsonify({'error': str(e)}), 500


    @app.route('/api/route/gpx-coordinates/<int:route_id>')
    @login_required  
    def get_route_gpx_coordinates(route_id):
        """API para obtener coordenadas específicamente del archivo GPX"""
        try:
            route = Route.query.get_or_404(route_id)
            ...
            print(f"Error en get_route_gpx_coordinates: {e}")
            return jsonify({'error': str(e)}), 500

    def extract_coordinates_from_html(file_path):
        """Extraer coordenadas de un archivo HTML de mapa"""
        try:
            ...
            patterns = [
                # Leaflet polyline pattern
                r'L\.polyline\(\[(.*?)\]',
                # Array de coordenadas
                r'var\s+\w+\s*=\s*\[((?:\[[-\d.,\s]+\],?\s*)+)\]',
                # Folium polyline pattern
                r'"coordinates":\s*\[((?:\[[-\d.,\s]+\],?\s*)+)\]'
            ]
            ...
            for pattern in patterns:
                ...
                for match in matches:
                    try:
                        ...
                        # Encontrar pares de coordenadas [lat, lng]
                        coord_pairs = re.findall(r'\[([-\d.]+),\s*([-\d.]+)\]', coords_text)
                        
                        if coord_pairs:
                            coordinates = [[float(lat), float(lng)] for lat, lng in coord_pairs]
                            if len(coordinates) > 1:  # Validar que hay una ruta válida
                                return coordinates
